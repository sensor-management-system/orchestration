...
<template>
  <div>
    ...
    >
      <v-card>
        <v-card-title>Suggest new compartment</v-card-title>
        <v-card-text>
          <p>
            You can suggest a new compartment for the controlled vocabulary. This is submitted as a proposal.
          </p>
          <p>
...
export default class CompartmentDialog extends mixins(Rules) {
  ...
  resetInputs () {
    this.suggestion = new Compartment()
  }
  ...
  notInExistingNames (term: string| null): boolean | string {
    ...
    if (this.compartments.find(x => x.name === term)) {
  ...
  async submit () {
    ...
    try {
      const result = await this.addCompartment({ compartment })
