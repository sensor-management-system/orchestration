...
export class StaticLocationEndActionSerializer {
  convertJsonApiRelationshipsModelList (
  ...
  ): StaticLocationEndAction[] {
    const actionIds = []

    if (relationships.configuration_static_location_end_actions) {
      const actionObject = relationships.configuration_static_location_end_actions as IJsonApiEntityWithoutDetailsDataDictList
    ...
    if (included && included.length > 0) {
      for (const includedEntry of included) {
        if (includedEntry.type === 'configuration_static_location_end_action') {
          if (actionIds.includes(includedEntry.id) && includedEntry.relationships) {
            if (includedEntry.attributes) {
              ...
              const action = StaticLocationEndAction.createFromObject({
                id: includedEntry.id || '',
