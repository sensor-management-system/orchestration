...
export class StaticLocationBeginActionSerializer {
  convertJsonApiRelationshipsModelList (
    ...
    if (relationships.configuration_static_location_begin_actions) {
      const actionObject = relationships.configuration_static_location_begin_actions as IJsonApiEntityWithoutDetailsDataDictList
    ...
    if (included && included.length > 0) {
      for (const includedEntry of included) {
        if (includedEntry.type === 'configuration_static_location_begin_action') {
          if (actionIds.includes(includedEntry.id) && includedEntry.relationships) {
            if (includedEntry.attributes) {
              ...
              const action = StaticLocationBeginAction.createFromObject({
                ...
                x: includedEntry.attributes.x,
                y: includedEntry.attributes.y,
                z: includedEntry.attributes.z
              })
              result.push(action)
            }
          }
