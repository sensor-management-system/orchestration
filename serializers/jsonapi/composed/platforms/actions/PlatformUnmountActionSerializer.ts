...
export class PlatformUnmountActionSerializer {
  private platformUnmountActionBasicDataSerializer: PlatformUnmountActionBasicDataSerializer
  ...
  constructor () {
    this.platformUnmountActionBasicDataSerializer = new PlatformUnmountActionBasicDataSerializer()
  ...
  convertJsonApiObjectListToModelList (jsonApiObjectList: IJsonApiEntityListEnvelope): PlatformUnmountAction[] {
    ...
    for (const platformUnmountActionPayload of data) {
      const platformUnmountActionBasic = this.platformUnmountActionBasicDataSerializer.convertJsonApiDataToModel(platformUnmountActionPayload)

      const relationships = platformUnmountActionPayload.relationships as IJsonApiRelationships
      ...
      if (configurationByIdLookup[configurationId] && contactByIdLookup[contactId]) {
        ...
        result.push(
          new PlatformUnmountAction(platformUnmountActionBasic, configurationBasicData, contactBasicData)
        )
      }
    }
