...
<template>
  <div>
    ...
    />
    <v-card>
      <!-- Edit dynamic location view -->
      <v-card-title>Begin of the dynamic location</v-card-title>
      <v-card-text>
      ...
      <v-card-actions v-if="$auth.loggedIn">
        ...
        <v-btn color="green" small @click="saveEditedDynamicLocation">
          Apply
        </v-btn>
      </v-card-actions>
      <template v-if="endAction">
        <v-divider class="mx-4 mt-4" />
        <v-card-title>End of the dynamic location</v-card-title>
...
import ConfigurationDynamicLocationBeginActionDataForm from '@/components/configurations/ConfigurationDynamicLocationBeginActionDataForm.vue'
...
export default class DynamicLocationBeginActionEdit extends mixins(Rules) {
  ...
  private endAction: DynamicLocationEndAction | null = null
  ...
  initializeConfigurationAndAction (configuration: Configuration) {
    ...
    const beginAction = this.configuration.dynamicLocationBeginActions.find(action => action.id === this.actionId)
    if (beginAction) {
      ...
      if (endAction instanceof DynamicLocationEndAction) {
  ...
  get allActiveDevices (): Device[] {
    ...
    const tree = buildConfigurationTree(this.configuration, this.beginDate)
  ...
  async saveEditedDynamicLocation () {
    ...
    try {
      ...
      this.configuration.dynamicLocationBeginActions = this.configuration.dynamicLocationBeginActions.filter(x => x.id !== this.beginAction.id)
      this.configuration.dynamicLocationBeginActions.push(this.beginAction)
