...
export default class DynamicLocationBeginActionNew extends mixins(Rules) {
  ...
  get allActiveDevices (): Device[] {
    const tree = buildConfigurationTree(this.configuration, this.beginAction?.beginDate || this.selectedDate)
  ...
  async saveNewDynamicLocation () {
    ...
    try {
      ...
      this.isSaving = false
      this.closeNewDynamicLocationForm()
    } catch (e) {
