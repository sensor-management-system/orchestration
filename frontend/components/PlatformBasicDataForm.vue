      ...
      <v-col cols="12" md="3">
        ...
        >
          <template #append-outer>
            ...
            <a v-if="valuePlatformTypeItem && valuePlatformTypeItem.uri" target="_blank" :href="valuePlatformTypeItem.uri" style="line-height: 2;">
              <v-icon small>
                mdi-open-in-new
              </v-icon>
            </a>
        ...
        >
          ...
          <template v-if="generateSerialNumberActive" slot="append-outer">
            <v-tooltip right>
              ...
              <span>Generate a serial number if there is none provided by the manufacturer</span>
            </v-tooltip>
          </template>
        </v-text-field>
      </v-col>
...
export default class PlatformBasicDataForm extends mixins(Rules, ExternalUrlLinkMixin) {
  ...
  get serialNumberHint () {
    if (this.isSerialNumberInUse) {
      return 'Another platform already has an equal serial number.'
    }
    if (this.value.hasSystemGeneratedSerialNumber) {
      return 'Serial number was generated by the system.'
    }
    return ''
  }
