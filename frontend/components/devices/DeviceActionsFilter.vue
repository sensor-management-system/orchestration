    ...
    <v-row>
      <v-col>
        <v-select
          ...
          :items="deviceActionTypeItemsIncludingMounts"
          :item-text="(x) => x.name"
          return-object
          clearable
        >
...
import { AvailableContactsOfActionsGetter, AvailableYearsOfActionsGetter, DeviceFilter, IOptionsForActionType } from '@/store/devices'
...
@Component({
  ...
  computed: {
    ...mapGetters('devices', ['availableYearsOfActions', 'availableContactsOfActions']),
    ...mapGetters('vocabulary', ['deviceActionTypeItemsIncludingMounts'])
  }
})
export default class DeviceActionsFilter extends Vue {
  @Prop({
    required: true,
    type: Object
  })
