  ...
  <div>
    ...
    >
      <v-card v-if="showDialog">
        <v-card-title>Suggest new device type</v-card-title>
        <v-card-text>
          <p>
            You can suggest a new device type for the controlled vocabulary. This is submitted as a proposal.
          </p>
          <p>
          ...
          <v-form ref="suggestionForm" @submit.prevent>
            <v-row>
              <v-col>
                ...
                />
                <SimilarTermsList :search="newDeviceType.name" :terms="devicetypes" />
              </v-col>
            </v-row>
            <v-row>
              <v-col>
                <v-textarea
                  v-model="newDeviceType.definition"
                  label="Definition"
                  rows="2"
                />
              </v-col>
            ...
            <v-row>
              <v-col>
                <v-text-field
                  v-model="newDeviceType.provenance"
                  label="Provenance"
                />
              </v-col>
              <v-col>
                <v-text-field
                  v-model="newDeviceType.provenanceUri"
            ...
            <v-row>
              <v-col>
                <v-text-field
                  v-model="newDeviceType.category"
                  label="Category"
                />
              </v-col>
            </v-row>
            ...
            <v-row>
              <v-col>
                <v-select
                  v-model="newDeviceType.globalProvenanceId"
...
import { AddDeviceTypeAction, LoadGlobalProvenancesAction, VocabularyState } from '@/store/vocabulary'
...
export default class DeviceTypeDialog extends mixins(Rules) {
  ...
  notInExistingNames (term: string| null): boolean | string {
    ...
    if (this.devicetypes.find(dt => dt.name === term)) {
  ...
  async submit () {
    ...
    try {
      const result = await this.addDevicetype({ devicetype })
  ...
  async onOpenCloseToggle (newValue: boolean) {
    if (newValue) {
      // We open the dialog
      this.newDeviceType.name = this.initialTerm
