  ...
  <div>
    ...
    >
      <v-card v-if="showDialog">
        <v-card-title>Suggest new aggregation type</v-card-title>
        <v-card-text>
          <p>
            You can suggest a new aggregation type for the controlled vocabulary. This is submitted as a proposal.
          </p>
          <p>
          ...
          <v-form ref="suggestionForm" @submit.prevent>
            <v-row>
              <v-col>
                ...
                />
                <SimilarTermsList :search="suggestion.name" :terms="aggregationtypes" />
              </v-col>
            </v-row>
            <v-row>
              <v-col>
...
export default class AggregationTypeDialog extends mixins(Rules) {
  ...
  resetInputs () {
    this.suggestion = new AggregationType()
  }
  ...
  notInExistingNames (term: string| null): boolean | string {
    ...
    if (this.aggregationtypes.find(x => x.name === term)) {
  ...
  async submit () {
    ...
    try {
      const result = await this.addAggregationType({ aggregationType })
