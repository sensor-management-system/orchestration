  ...
  <div>
    ...
    >
      <v-card v-if="showDialog">
        <v-card-title>Suggest new contact role</v-card-title>
        <v-card-text>
          <p>
            You can suggest a new contact role for the controlled vocabulary. This is submitted as a proposal.
          </p>
          <p>
          ...
          <v-form ref="suggestionForm" @submit.prevent>
            <v-row>
              <v-col>
                ...
                />
                <SimilarTermsList :search="suggestion.name" :terms="cvContactRoles" />
              </v-col>
            </v-row>
            <v-row>
              <v-col>
...
import { AddCvContactRoleAction, LoadGlobalProvenancesAction, VocabularyState } from '@/store/vocabulary'
...
export default class ContactRoleDialog extends mixins(Rules) {
  ...
  resetInputs () {
    this.suggestion = new CvContactRole()
  }
  ...
  notInExistingNames (term: string| null): boolean | string {
    ...
    if (this.cvContactRoles.find(x => x.name === term)) {
  ...
  async submit () {
    ...
    try {
      const result = await this.addCvContactRole({ contactRole: suggestion })
