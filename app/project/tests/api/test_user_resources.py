...
class TestUserServices(BaseTestCase):
    ...
    def test_fail_post_a_contact(self):
    ...
    def test_fail_patch_a_contact(self):
    ...
    def test_fail_delete_a_contact(self):
        """Ensure using delete request will fail."""

        user = add_user()
        with self.client:
            response = self.client.delete(
                f"{self.url}/{user.id}", content_type="application/vnd.api+json",
            )
        self.assertEqual(response.status_code, 405)
