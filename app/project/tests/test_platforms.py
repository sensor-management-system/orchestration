...
class TestPlatformServices(BaseTestCase):
    ...
    def test_add_platform_model(self):
        ...
        platform = Platform(id=13,
                            ...
                            long_name='long_name test', manufacturer_name='manufacturer_name test',
        ...
        p = db.session.query(Platform).filter_by(
            id=platform.id).one()
    ...
    def add_platform_contacts_relationship(self):
        ...
        data = super().add_object(
            url=self.platform_url + "?include=contacts", data_object=platform_data,
            object_type=self.object_type)

        self.assertIn(contact.id,
                      data['data']['relationships']['contacts']['data']['id'])
