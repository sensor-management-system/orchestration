...
class Contact(db.Model):
    ...
    devices = db.relationship(
        'Device', secondary=device_contacts, lazy='subquery',
        backref=db.backref('contacts', lazy=True)
    )
    platforms = db.relationship(
        'Platform', secondary=platform_contacts, lazy='subquery',
        backref=db.backref('contacts', lazy=True)
    )
    configurations = db.relationship(
        "Configuration", secondary=configuration_contacts, lazy='subquery',
        backref=db.backref('contacts', lazy=True)
    )
