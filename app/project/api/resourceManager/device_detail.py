...
def delete_attachment_with_device_id(device_id_attend_to_delete):
    """
    use the minio class to delete an attachment or a list of attachments
    :param device_id_attend_to_delete:
    :return:
    """
    attachments_related_to_device = (
        db.session.query(DeviceAttachment)
        .filter_by(device_id=device_id_attend_to_delete)
        .all()
    )
    minio = FlaskMinio()
    if isinstance(attachments_related_to_device, list):
        for attachment in attachments_related_to_device:
            minio.remove_an_object(attachment.url)
    else:
        minio.remove_an_object(attachments_related_to_device.url)
...
class DeviceDetail(ResourceDetail):
    ...
    def before_delete(self, args, kwargs):
        """
        Delete the device attachment at the minio server
        :param args:
        :param kwargs:
        :return:
        """
        device_id_attend_to_delete = kwargs.get("id")
        delete_attachment_with_device_id(device_id_attend_to_delete)
        return kwargs
